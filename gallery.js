...
// Removed duplicate declaration of galleryData
  // Initialize gallery when DOM loads
  document.addEventListener('DOMContentLoaded', function() {  // Fixed typo: removed extra 'd'
    const galleryGrid = document.querySelector('.gallery-grid');
    
    if (!galleryGrid) {  // Safety check
    ...
    galleryData.forEach(artwork => {
      ...
      item.className = `gallery-item ${artwork.customClass || ''}`;
      
      // Debug: Log image paths
      console.log(`Loading image: ${artwork.image}`); 
